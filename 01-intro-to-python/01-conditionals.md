...
For this project we're going to need to be able to generate random numbers between 0 and 100.  We can do this by using the `randint` function from the `random` module.
...
This is where things get tricky though.  To be able to compare the user input to the random number generated, we need to convert it into a number as well.  We can use the `int` function for that, but if you pass it a badly formatted string, it will cause an error.
...
Now we're at the point where we need to have our program do one of two things.  If the user has given us numeric input, we can convert it to an integer safely. But if they haven't, then we can't.
